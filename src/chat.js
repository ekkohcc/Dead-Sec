...
require("../blessed-emoji-patch.cjs");
...
function ansiToTags(s) {
  return stripCtlAnsi(String(s)).replace(/\x1b\[([0-9;]*)m/g, (m, code) => {
    ...
    if (parts.includes("2")) {
      ...
      if (rest && ANSI_TAG[rest]) return `{gray-fg}{${ANSI_TAG[rest]}-fg}`;
      return "{gray-fg}";
    }
...
function sanitizeTui(text) {
  ...
  return escapeSpecials(ansiToTags(escapeBraces(cleaned)));
}
      ...
      return new Promise((resolve) => {
        ...
        const onKey = (ch, key) => {
          ...
          if (a === "y" || a === "a" || a === "n") {
            ...
            inputBox.clearValue();
            appendChatLine(`{gray-fg}→ ${a === "y" ? "允许一次" : a === "a" ? "本会话允许" : "拒绝"}{/gray-fg}`);
            spinner.resume();
            resolve(a !== "n");
          }
        };
...
const out = (s) => {
  ...
  for (const ln of String(s).split("\n")) {
    if (ln.trim()) appendChatLine(`{gray-fg}${sanitizeTui(ln)}{/gray-fg}`);
  }
};
...
const handleInput = (line) => {
  ...
  const cmdState = { active: false, sel: 0, rows: [], names: [] };
  ...
  const renderCmdPanel = () => {
    ...
    const body = cmdState.rows.map((r, i) =>
      i === cmdState.sel ? `{#3a3a6e-bg,white-fg}▶ ${r}{/}` : `  ${r}`
    ).join("\n");
  ...
  const openSessionPanel = () => {
    const groups = listSessions();
    if (!groups.length) {
      appendChatLine("{gray-fg}（还没有历史会话，先聊几句吧）{/gray-fg}");
      return;
    }
  ...
  appendChatLine(`{gray-fg}── Welcome to Dead-Sec │ {white-fg}${cfg.model}{/white-fg} │ 输入 /help 查看命令 ──{/gray-fg}`);
  appendChatLine("{gray-fg}💡 复制: 鼠标在聊天区按住左键拖动选中, 松开后单击一次即复制; 或 Shift+↑/↓ 选行 + Ctrl+K 复制 (右上角提示 Copied){/gray-fg}");
  ...
  inputBox.key(["enter"], (ch, key) => {
    ...
    if (cmdState.active && cmdState.rows.length) {
      const name = cmdState.names[cmdState.sel] || "help";
